...

{
	...
	function __construct($appsecret,$appid,$key) 
	...
	function createOauthUrlForCode($redirectUrl,$state="STATE#wechat_redirect")

	{
		...
		$urlObj["state"] = $state;
		file_put_contents(dirname(__FILE__).'/1.txt', $urlObj);
...
/**
	 * 	作用：生成可以获得openid的url
	 */
	function create_openid($appid,$appsecret,$code)
	{
		$urlObj["appid"] = $appid;
...

	{
		...
		$msg="Mozilla/5.0(iphone;CPU iphone OS 5_1_1 like Mac OS X) AppleWebKit/534.46(KHTML,like Geocko) Mobile/9B206 MicroMessenger/5.0";
		...
		curl_setopt($ch, CURLOPT_USERAGENT,$msg);
...
class UnifiedTransfers_pub extends Wxpay_client_pub

{
    ...
    {
        $xml = $this->createXml();
        $this->response = $this->postXmlCurl($xml,$this->url,true,$this->curl_timeout);
        return $this->response;
