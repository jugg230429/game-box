...
class RedisController
{
    ...
    public function __construct($config,$attr=array())
    {
        $attr =  [
                'timeout'=>10000000,
                'db_id'=>cmf_get_option('site_info')['dbid']?:10
        ];
        $this->dbId = cmf_get_option('site_info')['dbid']?:10;
        $this->attr        =    array_merge($attr,$attr);
        $this->redis    =    new \Redis();
        $this->port        =    $config['port'] ? $config['port'] : 6379;
        $this->host        =    $config['host'];
        $this->redis->connect($this->host, $this->port, $this->attr['timeout']);
        $this->auth = cmf_get_option('site_info')['redis_password']?:'';
        if($this->auth)
        {
            $this->redis->auth($this->auth);
        }
        $result = $this->redis->select($this->dbId);
        if(!$result){
            die("REDIS 连接失败,请检查配置");
        }
    ...
    public static function getInstance($config, $attr = array())
    {
        ...
        $attr['db_id']    =   cmf_get_option('site_info')['dbid']?:10;
    ...
    **/
    ...
    public function incr($auth)
    {
        return $this->redis->incr($auth);
    }
    /*********************自定义的方法,用于简化操作************************/
