...
{
    //总共数量
    protected function getAllButton($text = ""){
        return "<li class='form-control form-record'>共{$text}条记录</li>";
    }
    protected function getAllRow($text = ""){
        $module = request()->module();
        if($module != 'media'){
            $row = (int)input('request.row');
            $pp = "<select class='form-control jspagerow' style='width:120px;'><option vv='10' class='op10'";
            $pp .= ">每页10条</option><option vv='25' class='op25'";

            if($row==25){
                $pp .= " selected >每页25条</option><option vv='50' class='op50'";
            }else{
                $pp .= ">每页25条</option><option vv='50' class='op50'";
            }

            if($row==50){
                $pp .= " selected >每页50条</option><option vv='100' class='op100'";
            }else{
                $pp .= ">每页50条</option><option vv='100' class='op100'";
            }

            if($row==100){
                $pp .= " selected >每页100条</option></select>";
            }else{
                $pp .= ">每页100条</option></select>";
            }
        }
        return $pp;
    }
