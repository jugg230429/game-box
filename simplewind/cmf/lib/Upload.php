...
class Upload
{
    ...
    public function upload($local=0,$tmp_flag=0)
    {
        ...
        $app = $this->request->post('app')?$this->request->post('app'):$this->app;//修改核心  可修改类元素
        ...
        $exts =  pathinfo($originalName,PATHINFO_EXTENSION);//获取后缀
        ...
        $arrInfo = [];
        if(in_array($fileImage->getExtension(),['jpg','jpeg','png','gif','bmp4'])){
            $imginfo = getimagesize($fileImage->getPathname());
            if($imginfo[0]>=$imginfo[1]){
                $info['type'] = 0;
            }else{
                $info['type'] = 1;
            }
            $arrInfo["more"] = json_encode($info);
        }
...
//        }
        ...
        if ($storage['type'] != 'Local' && empty($local)) { //  增加存储驱动
            ...
            if ($needUploadToRemoteStorage) {
                session_write_close();
                // $tmp_flag = 2;
                $result = $storage->upload($arrInfo["file_path"], './upload/' . $arrInfo["file_path"], $fileType,'',$tmp_flag);
                $upload_code = $result['code'] ?? 0;
                if($upload_code == -2){
                    // 上传失败
                    return $result;
                    exit;
