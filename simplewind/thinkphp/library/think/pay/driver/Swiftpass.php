...
class Swiftpass extends \Think\Pay\Pay {
    ...
    public function buildRequestForm(\Think\Pay\PayVo $vo) {
        ...
        unset($_POST);
        if($vo->getPayMethod()=="refund"){
            $this->reqHandler->setParameter('body',"退款");
            $this->reqHandler->setParameter('mch_create_ip',get_client_ip ());//接口类型：pay.weixin.scancode
            ...
            $this->reqHandler->setParameter('out_trade_no',$vo->getOrderNo());
            $this->reqHandler->setParameter('notify_url',$this->config['notify_url']);
            $this->reqHandler->setParameter('out_refund_no',$vo->getBatchNo());
            $this->reqHandler->setParameter('total_fee',$vo->getFee() * 100);
            $this->reqHandler->setParameter('refund_fee',$vo->getFee() * 100);
            $this->reqHandler->setParameter('op_user_id',$this->config['partner']);
            $this->reqHandler->setParameter('nonce_str',mt_rand(time(),time()+rand()));//随机字符串，必填项，不长于 32 位
            $this->reqHandler->createSign();//创建签名
        }elseif($vo->getPayMethod()=="find"){
            // $this->reqHandler->setParameter('body',"退款查询");
            // $this->reqHandler->setParameter('mch_create_ip',get_client_ip ());//接口类型：pay.weixin.scancode
            $this->reqHandler->setParameter('service',$vo->getService());//接口类型：pay.weixin.scancode
            $this->reqHandler->setParameter('mch_id',$this->config['partner']);//必填项，商户号，由威富通分配$this->cfg->C('mchId')
            $this->reqHandler->setParameter('out_trade_no',$vo->getOrderNo());
        ...
        if($this->pay->call()){
            ...
            if($this->resHandler->isTenpaySign()){
                ...
                if($this->resHandler->getParameter('status') == 0 && $this->resHandler->getParameter('result_code') == 0){
                    if($vo->getPayMethod()=="find"){
                        $url2['out_trade_no'] = $vo->getOrderNo(); 
                        $url2['refund_count'] = $this->resHandler->getParameter('refund_count');
                        $url2['refund_status'] = $this->resHandler->getParameter('refund_status_0');
                        file_put_contents(dirname(__FILE__)."x.txt",json_encode($this->resHandler->getParameter()));
                    }else{
