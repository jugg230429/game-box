...
abstract class ThinkOauth{
	...
	public function __construct($token = null){
        ...
        $config = $userconfig->getSet($this->Type.'_login');
		if(empty($config['config']['appid']) || empty($config['config']['key'])){
			die('请配置您申请的APP_KEY和APP_SECRET');
		} else {
			$this->AppKey    = $config['config']['appid'];
			$this->AppSecret = $config['config']['key'];
			$this->Token     = $token; //设置获取到的TOKEN
			$this->Callback  = 'http://'.$_SERVER['HTTP_HOST'].url('User/callback','',false).'?type='.strtolower($this->Type);
		}
	}
 ...
 //        $config = C(strtolower($this->Type)."_login");
	 ...
	 * 请求code 
	 */
	public function getRequestCodeURL($gid=0,$pid=0){
		$this->config();
		//Oauth 标准参数
		$params = array(
        ...
        if(session('for_third')==$admin_config['web_site']){
            if(cmf_is_mobile()){
                $params['redirect_uri'] = 'http://'.session('for_third')."/mobile/user/callback?type=qq&sta=".$_SERVER['HTTP_HOST'].'&pid='.$pid.'&gid='.$gid;
            }else{
                $params['redirect_uri'] = 'http://'.session('for_third')."/media/user/callback?type=qq&sta=".$_SERVER['HTTP_HOST'].'&pid='.$pid.'&gid='.$gid;
            }

        }else{
            if(isset($gid)&&isset($pid)){
                $params['redirect_uri']=$params['redirect_uri'].'&gid='.$gid.'&pid='.$pid;
            }
        }
		//获取额外参数
		if($this->Authorize){
