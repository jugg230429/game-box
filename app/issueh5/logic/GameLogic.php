...
class GameLogic extends  HomeBaseController
{
    public function play($request)
    {
        if(empty($request['channel_code'])){
            ...
            $platformData = $mPlatform->field('id,open_user_id,platform_config_h5,service_config,controller_name_h5,status')->where('id','=',$request['channel_code'])->find();
            if($platformData['status']!=1){
                return json(['code'=>1014,'msg'=>'分发平台状态已关闭']);
            }
            //用户状态
            $mOpenUser = new OpenUserModel();
            $openUserData = $mOpenUser -> field('id,account,auth_status,status') -> where('id', '=', $platformData['open_user_id']) -> find();
        ...
        if($applyData['status']!=1||$applyData['enable_status']!=1){//不存在该分发或分发状态不符合
            ...
            if(!$controller_name){//渠道接分发
                ...
                if(!file_exists(APP_PATH.'issueh5/logic/pt/'.$controller_name.'Logic.php')){
                ...
                $param['channel_code'] = $request['channel_code'];
                $param['platform_user'] = 'sue_'.$user->id;
                $param['birthday'] = $user->birthday;
                $param['game_id'] = $request['game_id'];
                $login_url = $this->get_login_url($param);
                if(!$login_url){
                    return json(['code'=>1000,'msg'=>'系统错误']);
                }
                $this->assign('login_url',$login_url);
    ...
    public function controller_name($channel_code)
    {
        ...
        $viewData = $modelPlat
            ->field('controller_name_h5')
            ->where(array('id' => $channel_code))
            ->find();
        return $viewData->controller_name_h5?:false;
    }
    ...
    public function get_login_url(array $params)
    {
        ...
        ksort($data);//字典排序
        if(!file_exists(APP_PATH.'sdkh5/logic/GameLogic.php')){
            return '';
        }
        $data['sign'] = (new \app\sdkh5\logic\GameLogic()) -> h5SignData($data, $modelGameData['game_key']);
