...
class RbacController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $data = Db::name('role')->order(["list_order" => "ASC", "id" => "DESC"])->paginate($row);
        // 获取分页显示
        $page = $data->render();
    ...
    public function roleAddPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if ($result) {
                    write_action_log("新增用户组【" . $data['name'] . "】");
        ...
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                if (Db::name('role')->update($data) !== false) {
                    write_action_log("编辑用户组【" . $data['name'] . "】");
    ...
    public function roleDelete()
    {
        ...
        if ($count > 0) {
            ...
            if (!empty($status)) {
                write_action_log("删除用户组【" . $role_name . "】");
    ...
    public function authorize()
    {
        ...
        foreach ($result as $n => $t) {
            //后台首页不显示在前台选择
            if(($t['controller']=='main'&&$t['action']=='index')){
            ...
            }elseif($t['controller']=='AdminIndex' ||$t['controller']=='RecycleBin' || $t['controller']=='AdminAsset'){
    ...
    public function authorizePost()
    {
        if ($this->request->isPost()) {
            ...
            if (is_array($this->request->param('menuId/a')) && count($this->request->param('menuId/a')) > 0) {
                ...
                write_action_log("编辑用户组【" . get_role_name_by_id($roleId) . "】授权权限");
