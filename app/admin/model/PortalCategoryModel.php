...
class PortalCategoryModel extends Model
{
     ...
     * @throws \think\exception\DbException
     */
    public function adminCategoryTableTree($currentIds = 0, $tpl = '', $category = '')
    {
//        if (!empty($currentCid)) {
//            $where['id'] = ['neq', $currentCid];
//        }
        $categories = $this->order("list_order ASC")->where('delete_time', 0)->select()->toArray();
        $category_info = $this->where('id', $category)->find();
        $tree = new Tree();
        $tree->icon = ['&nbsp;&nbsp;│', '&nbsp;&nbsp;├─', '&nbsp;&nbsp;└─'];
        $tree->nbsp = '&nbsp;&nbsp;';
        ...
        foreach ($categories as $item) {
            ...
            $item['disabled'] = $item['parent_id'] ? '' : 'disabled';
            if ((in_array($category, [2, 3, 4, 5]) && in_array($item['id'], [7])) || (in_array($category, [7]) && in_array($item['id'], [2, 3, 4, 5]))) {
                continue;
            }
            $item['class'] = $item['parent_id'] ? 'data-item-tr' : '';
            //$item['url']            = cmf_url('portal/List/index', ['id' => $item['id']]);
//            $item['str_action'] = '<a class="btn btn-xs btn-primary" href="' . url("Category/add", ["parent" => $item['id']]) . '">添加子分类</a> ';
            $item['str_action'] .= ' <a class="" href="' . url("Category/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a> ';
//            $item['str_action'] .= ' <a class="btn btn-xs btn-danger js-ajax-delete" href="' . url("Category/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
//            if ($item['status']) {
//                $item['str_action'] .= '<a class="btn btn-xs btn-warning js-ajax-dialog-btn" data-msg="您确定隐藏此分类吗" href="' . url('Category/toggle', ['ids' => $item['id'], 'hide' => 1]) . '">隐藏</a>';
//            } else {
//                $item['str_action'] .= '<a class="btn btn-xs btn-success js-ajax-dialog-btn" data-msg="您确定显示此分类吗" href="' . url('Category/toggle', ['ids' => $item['id'], 'display' => 1]) . '">显示</a>';
//            }
            array_push($newCategories, $item);
        }
