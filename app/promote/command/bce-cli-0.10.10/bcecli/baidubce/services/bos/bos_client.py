...
from bcecli.baidubce import bce_client_configuration
...
from bcecli.baidubce.services.bos import bos_handler
...
class BosClient(BceBaseClient):
    ...
    @required(bucket_name=(str, unicode), lifecycle=(dict))
    def set_bucket_lifecycle(self, bucket_name, lifecycle, config=None):
        """
        Set lifecycle
        ...
        :type lifecycle: dict of lifecyle rules
        :param lifecycle: None
        :return:
            **HttpResponse Class**
        """
        self._send_request(http_methods.PUT,
                           bucket_name,
                           body=json.dumps(lifecycle, indent=4, separators=(',', ': ')),
                           headers={http_headers.CONTENT_TYPE: http_content_types.JSON},
                           params={'lifecycle': ''},
                           config=config)
    ...
    def get_bucket_lifecycle(self, bucket_name, config=None):
        ...
        :param bucket: None
        :return:
            **json text of lifecycle**
        """
        return self._send_request(http_methods.GET,
                                bucket_name,
                                params={'lifecycle': ''},
                                config=config,
                                body_parser=bos_handler.keep_raw_response_body
                                )

    @required(bucket_name=(str, unicode), key=(str, unicode), body=(str, unicode))
    def process_command(self, bucket_name, key, body, config=None):
        """
        ...
        return self._send_request(http_methods.POST,
                                ...
                                config=config,
                                body = body,
                                body_parser=bos_handler.parse_data_process_response)
    ...
    @required(bucket_name=(str, unicode), logging=(dict))
    def set_bucket_logging(self, bucket_name, logging, config=None):
        """
        Set logging
        ...
        :type logging: dict of lifecyle rules
        :param logging: None
        :return:
            **HttpResponse Class**
        """
        self._send_request(http_methods.PUT,
                           bucket_name,
                           body=json.dumps(logging, indent=4, separators=(',', ': ')),
                           headers={http_headers.CONTENT_TYPE: http_content_types.JSON},
                           params={'logging': ''},
                           config=config)
    ...
    @required(bucket_name=(str, unicode), storage_class=(str, unicode))
    def put_bucket_storage_class(self, bucket_name, storage_class, config=None):
        """
        ...
        :return:
            **HttpResponse Class**
        """
        bucket_storage_class = {"storageClass":storage_class}
        self._send_request(http_methods.PUT,
                           bucket_name,
                           body=json.dumps(bucket_storage_class, indent=4, separators=(',', ': ')),
                           headers={http_headers.CONTENT_TYPE: http_content_types.JSON},
                           params={'storageClass': ''},
                           config=config)
    ...
    def get_bucket_storage_class(self, bucket_name, config=None):
        ...
        :param bucket: None
        :return:
            **json text of bucket storage class**
        """
        return self._send_request(http_methods.GET,
                                bucket_name,
                                params={'storageClass': ''},
    ...
    @required(bucket_name=(str, unicode), canned_acl=(str, unicode))
    ...
    def _parse_bos_object(http_response, response):
        ...
        for k, v in http_response.getheaders():
            if k.startswith(http_headers.BCE_USER_METADATA_PREFIX):
                ...
                user_metadata[k.decode(bcecli.baidubce.DEFAULT_ENCODING)] = \
                    v.decode(bcecli.baidubce.DEFAULT_ENCODING)
    ...
    def get_object_meta_data(self, bucket_name, key, config=None):
        ...
        """
        def _parse_user_meta(http_response, response):
            user_metadata = {}
            for k, v in http_response.getheaders():
                if k.startswith(http_headers.BCE_USER_METADATA_PREFIX):
                    k = k[len(http_headers.BCE_USER_METADATA_PREFIX):]
    ...
    @required(bucket=(str, unicode), key=(str, unicode), data=(str, unicode))
    def put_object_from_string(self, bucket, key, data,
        ...
        if isinstance(data, unicode):
            data = data.encode(bcecli.baidubce.DEFAULT_ENCODING)
    ...
    @required(bucket=(str, unicode), key=(str, unicode), file_name=(str, unicode))
    ...
    @required(source_bucket_name=(str, unicode),
              source_key=(str, unicode),
              target_bucket_name=(str, unicode),
              target_key=(str, unicode))
    ...
    def _prepare_object_headers(
        ...
        if content_disposition is not None:
            headers[http_headers.CONTENT_DISPOSITION] = \
                utils.convert_to_standard_string(content_disposition)
        if expires is not None:
            headers[http_headers.EXPIRES] = utils.convert_to_standard_string(expires)
        if cache_control is not None:
            headers[http_headers.CACHE_CONTROL] = utils.convert_to_standard_string(cache_control)
    ...
    def _send_request(
        ...
        if config.credentials.security_token and config.credentials.security_token.strip():
            headers = headers or {}
            headers[http_headers.STS_SECURITY_TOKEN] = config.credentials.security_token
