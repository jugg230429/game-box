...
class CouponController extends BaseController
{
    ...
    public function lists()
    {
        ...
        $data = $logic->get_coupon_lists(PID,$request);
    ...
    public function grant_list()
    {
        ...
        if (!empty($testGameIds)) {
            $map['game_id'] = ['not in', $testGameIds];
        }
...
//                $discount = $lPay -> get_agent_discount($vo['pgame_id'], PID);
                foreach ($user as $k=>$v){
                    $discount = $lPay -> get_agent_discount($vo['pgame_id'], PID,$v['id']);
                    // $total += $vo['money']*$discount['discount']/10; // 原
                    $total += $vo['money']; //去掉折扣
                    $add[] = $this->get_coupon_data($vo,$v,$vo['money'],$game_id);//09-17 发代金券无折扣 yyh
                }
            }
