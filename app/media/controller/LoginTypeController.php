...
//仅用到qq、微信
class LoginTypeController extends BaseController
{
    //登录成功，获取腾讯QQ用户信息
    public function qq($token)
    {
        ...
        if ($data['ret'] == 0) {
            $userInfo['type'] = 'QQ';
            $userInfo['openid'] = get_union_id($token['access_token']);
            $userInfo['nickname'] = $data['nickname'];
            $userInfo['headpic'] = $data['figureurl_qq_1'];
            return $userInfo;
        } else {
    ...
    public function weixin($token)
    {
        ...
        $data = $weixin->call('user/userinfo');
        if ($data['ret'] == 0) {
