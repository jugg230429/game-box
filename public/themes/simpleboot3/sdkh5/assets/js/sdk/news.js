 ...
 */
  var news="";
    ...
    $("body").append(news) ;
    //获取文章数据
	ajax_post('/sdkh5/game/get_article','game_id='+game_id, function (result) {
			var html = '';
			var gonggao = result.gonggao;
			if(gonggao && gonggao.length) {
				for (var i in gonggao) {
					html += '<a href="javascript:;" data-id="'+gonggao[i].id+'" class="news-list  js-news-list">';
        ...
        if(huodong && huodong.length) {
            for (var i in huodong) {
                html += '<a href="javascript:;" data-id="'+huodong[i].id+'" class="news-list  js-news-list">';
        ...
        if(gonglue && gonglue.length) {
            for (var i in gonglue) {
                html += '<a href="javascript:;" data-id="'+gonglue[i].id+'" class="news-list  js-news-list">';
	...
	$(".js-news-list").click(function(){
		var id = $(this).data('id');
        ajax_post('/sdkh5/game/get_article_detail','id='+id, function (result) {
            $(".newsdetailpop").show();
						$(".newsdetailpop").find('.news-main').empty();
						$(".newsdetailpop").find('.news-main').html(result.post_content);
            // $(".newsdetailpop").find('.news-main').html(result.post_content).scrollTop(0);
