...
(function( $ ) {
    ...
    $.fn.extend({
        ...
        diyUpload:function( opt, serverCallBack ) {
            if ( typeof opt != "object" ) {
                if(opt.msg) opt.msg('参数错误!'); else alert('参数错误!');
                return;
            }
            ...
            if ( !WebUploader.Uploader.support() ) {
								if(opt.msg) opt.msg('上传组件不支持您的浏览器！!'); else alert('上传组件不支持您的浏览器！!');
                return false;
            }

            //绑定文件加入队列事件;
            webUploader.on('fileQueued', function( file ) {
                createBox( $fileInput, file ,webUploader,delCallBack);
            ...
            webUploader.on('uploadFinished', function(){
                //$fileInput.next('.diyButton').remove();
            });
            //绑定发送至服务端返回后触发事件;
            ...
            webUploader.on('uploadSuccess',function( file, response ){
                ...
                if ( successCallBack ) {
										$fileBox.find('.diyCancel').attr('data-url',response.path);
										$fileBox.find('img').attr('data-id',response.id);
                    successCallBack( response , $fileBox, webUploader, file.id);
                }
            });
            ...
            webUploader.on('error', function( code ) {
                ...
                if(opt.msg) opt.msg(text); else alert( text );
            });
             return webUploader;
        }
    });
    ...
    //创建文件操作div;
    function createBox( $fileInput, file, webUploader ,delCallbackFun) {
        var file_id = file.id;
        ...
        var li = '<li id="fileBox_'+file_id+'" class="diyUploadHover"> \
					<div class="viewThumb">\
					    ...
					    <p class="diyControlbox"><span class="diyCancel" data-url=""><i></i></span></p>\
					</div> \
				</li>';
        ...
        var $diyCancel = $fileBox.find('.diyCancel').one('click',function(){
						if(delCallbackFun) delCallbackFun($fileBox);
            removeLi( $(this).closest('.diyUploadHover'), file_id, webUploader );
        });
        ...
        webUploader.makeThumb( file, function( error, dataSrc ) {
            if ( !error ) {
                $fileBox.find('.viewThumb').append('<img data-id="" src="'+dataSrc+'" >');
            }
        });
    }
