...
function openUploadDialog(dialog_title, callback, extra_params, multi, filetype, app) {
    ...
    if(multi) {
        max = 5 ;
        if(extra_params && extra_params['max']) {
            max = extra_params['max'];
        }
        if(extra_params && extra_params['container']) {
            var container = extra_params['container'];
            var nowlength = $(container).find('li[id^="saved-"]').length
            if( nowlength >= max) {
    ...
    app        = app ? app : GV.APP;
    var params = '&max='+max+'&multi=' + multi + '&filetype=' + filetype + '&app=' + app;
    Wind.use("artDialog", "iframeTools", function () {
...
function uploadMultiImage(dialog_title, container_selector, item_tpl_wrapper_id, extra_params, app) {
    ...
    extra_params['container'] = container_selector;
    openUploadDialog(dialog_title, function (dialog, files) {
