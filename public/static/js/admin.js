;(function () {
                        ...
                        $form.ajaxSubmit({
                            ...
                            success: function (data, statusText, xhr, $form) {
                                ...
                                if (data.code == 1) {
                                    ...
                                    layer.close(btn_tijiao_load_img_id);
    ...
    if ($('a.js-ajax-dialog-btn-xz').length) {
        Wind.use('artDialog', 'noty', function () {
            $('.js-ajax-dialog-btn-xz').on('click', function (e) {
                e.preventDefault();
                var $_this  = this,
                    $this   = $($_this),
                    href    = $this.data('href'),
    ...
    if (bootstrapDateTimeInput.length) {
        Wind.css('bootstrapDatetimePicker');
        Wind.use('bootstrapDatetimePicker', function () {
            var endDate = bootstrapDateTimeInput.attr('data-endDate');
            if (endDate) {
...
function openUploadDialog(dialog_title, callback, extra_params, multi, filetype, app) {
    ...
    var string = '';
    if(extra_params){
        $.each(extra_params, function(n, value) {
...
function uploadPack(dialog_title, input_selector, filetype, extra_params, app) {
    filetype = filetype ? filetype : 'file';
    openUploadDialog(dialog_title, function (dialog, files) {
        $(input_selector).val(files[0].filepath);
        $(input_selector + '-preview').attr('href', files[0].preview_url);
