...
namespace plugins\baidu_bac\lib;


use BaiduBce\Services\Bos\BosClient;
use think\Controller;

class BaiduBac extends Controller
{
    ...
    {
        $pluginClass = cmf_get_plugin_class('BaiduBac');
    ...
    public function upload($file, $filePath, $fileType = 'image', $param = null,$tmp_flag)
    {
        ...
        try{
            ...
            $bucketName = $bucket;//bucket名字，相当于你在bucket创建好的那个文件夹，如果没有会自动创建。
            $client->putObjectFromFile('', $object, $filePath);
            $flag = 1;

        }catch(\Exception $e){
            // $e->getMessage();
            $flag = 0;
            // $this->error('百度云参数配置不正确');
            // exit;
        }
